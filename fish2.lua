local Config = {
    WindowName = "version DIG",
    Color = Color3.fromRGB(682,41,143),
        ...
        if string.match(v.Name, "key") then
            for i, model in pairs(v:GetDescendants()) do
                if model:IsA("Tool") then
                    if model.RarityLevel.Value >= 6 then
...
local Slider1 = Section2:CreateSlider("Boat Speed", 0,25000,nil,true, function(Value)
 for i, v in pairs(game.Workspace:GetChildren()) do
